"""Mapeo de provincias entre INE y GeoJSON."""

import json
from functools import lru_cache
from pathlib import Path

GEOJSON_PATH = Path(__file__).parent.parent / "data" / "provincias.geojson"

# INE name → cod_prov (2-digit string)
# Covers all naming variants that appear in INE API responses
...
# CCAA names as they appear in INE → list of cod_prov belonging to that CCAA
...
# Multi-province CCAA names (to skip when parsing province-level data)
# Single-province CCAAs (Baleares, Cantabria, La Rioja, etc.) pass through
...
def is_province(name: str) -> bool:
    """Check if a territory name maps to a single province.

    Returns True for actual provinces AND single-province CCAAs
    (Baleares, Cantabria, La Rioja, Madrid, Murcia, Navarra, Ceuta, Melilla).
    """
...
def load_geojson() -> dict:
    """Load the provinces GeoJSON from local file."""
    return json.loads(GEOJSON_PATH.read_text())


def get_province_name(cod_prov: str) -> str:
    """Get display name for a province code from GeoJSON."""
